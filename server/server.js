  ...
  const resolvers = {
    ...
    Query: {
      ...
      users: async (_, args, context) =>
        await db.collection("users").find({ roomId: context.roomId }).toArray(),
      cards: async (_, args, context) =>
        await db.collection("cards").find({ roomId: context.roomId }).toArray(),
    },
    Mutation: {
      createRoom: async () => {
        const room = {
      ...
      login: async (_, args, context) => {
        ...
        // in case
        const findRoomRes = await db
          .collection("rooms")
          .findOne({ id: ObjectId(context.roomId) });
        if (!findRoomRes) {
        ...
        if (!findRoomRes.hasAdmin) {
          await db
            .collection("rooms")
            .findOneAndUpdate(
              { id: ObjectId(context.roomId) },
              { $set: { hasAdmin: true } }
            );
        }
        const { result: insertUserRes } = await db
          .collection("users")
          .insertOne(user);
        if (!insertUserRes.ok) {
      ...
      deleteCard: async (_, args) => {
        const card = await db
          .collection("cards")
          .findOne({ id: ObjectId(args.id) });
        const { result, ops } = await db
          .collection("cards")
          .deleteOne({ id: ObjectId(args.id) });
        if (!result.ok) {
      ...
      switchSteps: async (_, args, context) => {
        ...
        await pubsub.publish(STEP_CHANGED, {
          stepChanged: { ...updatedRoom, step: updatedRoom.step + 1 },
        });
        return true;
      },
      leaveRoom: async (_, args, context) => {
        const user = await db
          .collection("users")
          .findOne({ id: ObjectId(context.userId) });
        if (user.role === "ADMIN") {
          await db
            .collection("rooms")
            .findOneAndUpdate(
              { id: ObjectId(user.roomId) },
              { $set: { done: true } }
            );
        }
        await db
          .collection("users")
          .deleteOne({ id: ObjectId(context.userId) });
